...
ls1 <- center %>%
  group_by(edate) %>%
  summarise(laufzeit_haus = mean(laufzeittage))%>%
  ungroup()
...
ggplot(combined_data_laufzeit, aes(x = edate, y = laufzeit_haus, color = location, group = location)) +
  #geom_line() +
  labs(title = "Development of validity period length",
       x = "Year",
       y = "average validity period") +
  theme_minimal() +
...
ggplot(combined_data, aes(x = month, y = list, color = location, group = location)) +
  geom_smooth() + 
  labs(title = "Active house listings",
       x = "Year",
       y = "Count of listings") +
